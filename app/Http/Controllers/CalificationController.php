    ...
    public function generateTemplateCalification(int $student_id){
        ...
        foreach ($califications as $row) {
                ...
                $table_html.='<tr class="row_note">';
                    $table_html.='<td>'.$row->area_name.'</td>';

                    $acum=0;
                    foreach ($row->periods as $p) {
                        $acum+=$p;
                        $table_html.='<td class="note">'.$p.'</td>';
                    }

                    $table_html.='<td class="note">'.($acum/count($row->periods)).'</td>';

                $table_html.='</tr>';
                $table_html.='<tr>';
                    $table_html.='<td class="teacher"> Profesor: '.$row->teacher_name.' '.$row->teacher_lastname.'</td>';

                    foreach ($row->periods as $p) {
                        $table_html.='<td class="note"></td>';
                    }
                ...
                foreach ($row->achievements as  $achievement) {
                    $acum_ach=0;
                    $table_html.='<tr>';
                        $table_html.='<td class="note_detail">'.$achievement->achievement.'</td>';

                        foreach ($row->periods as $p) {
                            $acum_ach+=(($p*$achievement->percentage)/100);
                            $table_html.='<td class="note_mark">'.(($p*$achievement->percentage)/100).'</td>';
                        }

                        $table_html.='<td class="note_mark">'.($acum_ach/count($row->achievements)).'</td>';
                    $table_html.='</tr>';
                    ...
                    foreach ( $achievement->indicators as $indicator) {
                        $acum_ind=0;
                        $table_html.='<tr>';
                            $table_html.='<td class="note_detail_2">'.$indicator->type_activity.'</td>';

                            foreach ($row->periods as $p) {
                                $achievement_percent=(($p*$achievement->percentage)/100);
                                $acum_ind+=(($achievement_percent*$indicator->activity_rate)/100);
                                $table_html.='<td class="note">'.(($achievement_percent*$indicator->activity_rate)/100).'</td>';
                            }

                            $table_html.='<td class="note">'.($acum_ind/count($achievement->indicators)).'</td>';
